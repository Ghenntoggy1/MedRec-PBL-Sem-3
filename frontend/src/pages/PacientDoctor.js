...
export default function PacientDoctor(){
  const textStyle = {
    fontSize: "1.4em",
    textAlign:'left',
    color:"#02825D" ,
  ...
  useEffect(() => {
    const fetchPatientData = async () => {
      try {
        const response = await data.fetchPatientInfo(pat_idnp.split("=")[1]);
  ...
  useEffect(() => {
    const fetchMedicalRecord = async () => {
      try {
        const response = await data.fetchMedicalRecord(pat_idnp.split("=")[1]);
    ...
    const fetchInstitutionName = async () => {
      try {
        const response = await data.fetchInstitutionName(pat_idnp.split("=")[1]);
  ...
  return(
    <SimpleGrid columns={2} spacing={10} minChildWidth="250px" height="700px">
      ...
      <div style={textStyle} mb={2}>
        ...
        <Text mb={2}>
          Centrul Medical: {institutionName ? institutionName : "N/A"}
        </Text>
        <Text mb={2} >
